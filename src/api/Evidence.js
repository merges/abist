...
function filterEntriesToPopulationOnly (entries) {
...
function filterEntriesWithNonPlaceboComparisons (entries) {
...
function getEntriesForMeasure (entries) {
  /*
...
function getSheetUrl (sheet) {
  ...
  let xmlListing = 'https://spreadsheets.google.com/feeds/worksheets/1AR88Qq6YzOFdVPgl9nWspLJrZXEBMBINHSjGADJ6ph0/private/full'

  let key  = '1AR88Qq6YzOFdVPgl9nWspLJrZXEBMBINHSjGADJ6ph0'
  let base = 'https://spreadsheets.google.com/feeds/list/'
       + key
       + '/'
  let end  = '/public/values?alt=json'
...
function processData (data) {
  ...
  _.each(data.feed.entry, function (value, index) {
    if (index == 0) {
      return
    }
    var entry = {}
...
function processTagDescriptions (data) {
  var tagDescriptions = {}
...
export {
  ...
  getEntriesForMeasure,
  getSheetUrl,
  processData,
  processGrades,
  processMeasures,
  processMetrics,
  processTagDescriptions,
}